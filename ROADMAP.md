...
> **Status (2026-08-27)** — Phase 1 in progress: API foundation and contracts.
> **Just shipped:** NestJS API skeleton, structured logging, health endpoints, and exception handling.
> **Next milestone:** OpenAPI generation + the first generated SDK flow consumed by `apps/web`.
> **Known foundation gaps:** Turborepo, `packages/*`, and local Docker infrastructure described below are not yet present.
...
- `apps/web` — Next.js 16 + Mantine/CSS Modules + React Query + `lightweight-charts` + Playwright + Vitest. Mantine is scheduled for replacement in Phase 1.5.
...
2. Add `packages/shared` and `packages/sdk` (deferred; `packages/*` workspace support starts in Phase 1.5).
3. Centralize linting, formatting, TS config, root scripts, and workspace conventions (partially complete; Turborepo is deferred to Phase 1.5).
